...
THaAnalyzer::Stage_t* THaAnalyzer::DefineStage( const Stage_t* item )
{
  if( !item || item->key < 0 )
    return NULL;
  // just an error check if no object or unsensible key given (Stage_t and its  key defined as enum in header file)
  if( item->key >= MAXSTAGE ) {      // MAXSTAGE is `sanity check' (=100 normally) to stop insane number of analysis stages
    Error("DefineStage", "Too many analysis stages.");
    return NULL;
  }
  if( item->key >= fNStages ) {  // final key in the InitStages definition of stagedef has final key of -1 (fail thsi coniditon and exit the loop) (fNstages redfined in this loop to iterate +1 every loop)
    Int_t newmax = item->key+1;
    Stage_t* tmp = new Stage_t[newmax];       // basically iterator through the key values of the Stage_t objects defined in InitStages and get increasingly larger arrays of Stage_t structs
    memcpy(tmp,fStages,fNStages*sizeof(Stage_t));
    // copies from fStages to temp, after one 'round' this should be fine but first stage looks like it copies a a NULL fStages which is not allowed (for memcpy src or dest can't be NULL pointer) 
    memset(tmp+fNStages,0,(newmax-fNStages)*sizeof(Stage_t));
    // the first value sets a pointer tmp (which is a pointer to a start of an array of Stage_t structs, the second value says to set the values there to zero, and the third value the number of bytes to be set
    delete [] fStages;
    fStages = tmp;
    fNStages = newmax;
    // effect of this if condition is simply to create larger array based on adding more Stage_t structs, with c++ arrays this must be done(? I think) by creating a new array (new size) and giv 
    // it an empty block at the ned to be filled.
  }
  return (Stage_t*)memcpy(fStages+item->key,item,sizeof(Stage_t));
  // this step puts the new Stage_t into the array
}
...
THaAnalyzer::Counter_t* THaAnalyzer::DefineCounter( const Counter_t* item )
{
  ...
  // exactly same idea as above function
  if( !item || item->key < 0 )
    return NULL;
...
void THaAnalyzer::InitStages()
{
  ...
  if( fNStages>0 )  return; // defaults fNStages = 0
...
Int_t THaAnalyzer::DoInit( THaRunBase* run )
{
  ...
  // fPrevEvent is initialised to NULL
  // in apex_2ARMs fEvent is initilaised before Process so this condition would fail
  if( fEvent != fPrevEvent ) {
    // this condition is true for APEX_2AR
  ...
  if( !fEvData || fEvData->IsA() != gHaDecoder ) {
    ...
    // gaHaDecoder is a CodaDecoder object 
...
Int_t THaAnalyzer::PhysicsAnalysis( Int_t code )
{
  ...
  if( fNev < fRun->GetFirstEvent() )
    // fNev = number of event
    // checks is event number is within range of interest (if you pick only 100 events etc)
